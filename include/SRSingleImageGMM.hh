...
class SRSingleImageGMM {
  ...
  public:
    ...
    static std::vector<Mat>* buildLPyramid(std::vector<Mat>* pyrH, float scale_factor, int isPara);
    static std::vector<Mat>* buildHPyramid(Mat h0, float scale_factor, int levels, int isPara);

  private:

    static Mat buildSampleData(std::vector<Mat>* pyrH, std::vector<Mat>* pyrL, int isPara);
